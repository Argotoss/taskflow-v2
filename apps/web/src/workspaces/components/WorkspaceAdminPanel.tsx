...
const WorkspaceAdminPanel = ({ accessToken, currentUserId }: WorkspaceAdminPanelProps): JSX.Element => {
  ...
  if (workspaces.length === 0) {
    return (
      <section className="workspace-card">
        <h3>Workspace access</h3>
        <p className="workspace-card__notice">Create a workspace to invite teammates and set roles.</p>
      </section>
    );
  }
        ...
        <div className="workspace-section">
          ...
          <ul className="workspace-members">
            {members.map((member) => {
              ...
              const disableRemove = (!isOwner && !isSelf) || (isSelf && member.role === 'OWNER' && members.filter((entry) => entry.role === 'OWNER').length === 1);
              return (
      ...
      <div className="workspace-section">
        ...
        <form className="workspace-invite" onSubmit={handleInviteSubmit}>
          ...
          <button type="submit" className="workspace-button" disabled={!canInvite || inviteSubmitting}>
            {inviteSubmitting ? 'Sending…' : `Invite to ${workspaceName}`}
          </button>
        </form>
      </div>
              ...
              <li key={invite.id} className="workspace-invites__item">
                ...
                <div className="workspace-invites__actions">
                  <button
                    type="button"
                    className="workspace-button workspace-button--ghost"
                    onClick={() => setInviteLink(buildInviteLink(invite.token))}
                  >
                    Copy link
                  </button>
                  <button
